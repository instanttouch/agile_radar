...
class BootStrap {
	...
	def init = { servletContext ->
		...
		try {
			...
			if(!goal){
				goal = new Category(name:'Goal',positive:true).save()
			} 
			
			def fun = Category.findByName('Fun')
			if(!fun){
				fun = new Category(name:'Fun',positive:true).save() 
			} 

			def wrong = Category.findByName('Wrong')
			if(!wrong){
				wrong = new Category(name:'Wrong',positive:true).save() 
			} 

			def impediments = Category.findByName('Impediments')
			if(!impediments){
				impediments = new Category(name:'Impediments',positive:true).save() 
			} 

			def quality = Category.findByName('Quality')
			if(!quality){
				quality = new Category(name:'Quality',positive:true) .save()
			} 
